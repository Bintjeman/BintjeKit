...
namespace {
    ...
    [[nodiscard]] SpriteTextureData getSpriteTextureData(const sf::Sprite &sprite) {
        ...
        return {
            toImVec2(textureRect.position.componentWiseDiv(textureSize)),
...
namespace ImGui {
    namespace SFML {
        ...
        bool Init(sf::Window &window, const sf::Vector2f &displaySize, bool loadDefaultFont) {
            s_currWindowCtx = s_windowContexts.emplace_back(
                std::make_unique<WindowContext>(&window)).get();
            ...
            // load mouse cursors
            const auto loadMouseCursor = [
                    ](ImGuiMouseCursor imguiCursorType, sf::Cursor::Type sfmlCursorType) {
                s_currWindowCtx->mouseCursors[imguiCursorType] = sf::Cursor::createFromSystem(
                    sfmlCursorType);
            };
        ...
        void SetCurrentWindow(const sf::Window &window) {
            auto found = std::find_if(s_windowContexts.begin(),
                                      ...
                                      [&](std::unique_ptr<WindowContext> &ctx) {
                                          return ctx->window->getNativeHandle() == window.
                                                 getNativeHandle();
                                      });
        ...
        void ProcessEvent(const sf::Window &window, const sf::Event &event) {
            ...
            assert(
                s_currWindowCtx && "No current window is set - forgot to call ImGui::SFML::Init?");
            ImGuiIO &io = ImGui::GetIO();
            ...
            if (s_currWindowCtx->windowHasFocus) {
                if (const auto *resized = event.getIf<sf::Event::Resized>()) {
                ...
                } else if (const auto *mouseButtonPressed = event.getIf<
                    sf::Event::MouseButtonPressed>()) {
                    const int button = static_cast<int>(mouseButtonPressed->button);
                    if (button >= 0 && button < 3) {
                        s_currWindowCtx->mousePressed[static_cast<int>(mouseButtonPressed->button)]
                                = true;
                        io.AddMouseButtonEvent(button, true);
                    }
                } else if (const auto *mouseButtonReleased = event.getIf<
                    ...
                    s_currWindowCtx->mouseMoved = false;
                } else if (const auto *mouseWheelScrolled = event.getIf<
                    sf::Event::MouseWheelScrolled>()) {
                    if (mouseWheelScrolled->wheel == sf::Mouse::Wheel::Vertical ||
                        (mouseWheelScrolled->wheel == sf::Mouse::Wheel::Horizontal && io.
                         KeyShift)) {
                ...
                } else if (const auto *joystickConnected = event.getIf<
                ...
                } else if (const auto *joystickDisconnected = event.getIf<
    ...
    if (!io.WantTextInput && s_currWindowCtx->wantTextInput)
    {
        ...
        s_currWindowCtx->wantTextInput = false;
            ...
            // gamepad navigation
            if ((io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) && s_currWindowCtx->joystickId
                != NULL_JOYSTICK_ID) {
        ...
        void Shutdown(const sf::Window &window) {
            LOGGER->debug("ImGui::SFML::Shutdown(const sf::Window &window)");
            LOGGER->flush();
        ...
        void Shutdown() {
            LOGGER->debug("ImGui::SFML::Shutdown()");
            LOGGER->flush();
            LOGGER->debug("ImGui::SFML::Shutdown() - s_currWindowCtx = nullptr;...");
            s_currWindowCtx = nullptr;
            LOGGER->debug("ImGui::SFML::Shutdown() - ImGui::SetCurrentContext(nullptr);...");
            ImGui::SetCurrentContext(nullptr);
            LOGGER->debug("ImGui::SFML::Shutdown() - s_windowContexts.clear();");
            s_windowContexts.clear();
            LOGGER->debug("ImGui::SFML::Shutdown() - done");
            LOGGER->flush();
        }
...
namespace {
    ...
    void RenderDrawLists(ImDrawData *draw_data) {
        ...
        assert(io.Fonts->TexID != (ImTextureID) nullptr);
        // You forgot to create and set font texture
    ...
    glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &last_element_array_buffer);
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++) {
            ...
            glColorPointer(4,
                           GL_UNSIGNED_BYTE,
                           sizeof(ImDrawVert),
                           (const GLvoid *) ((const char *) vtx_buffer + offsetof(
                                                 ImDrawVert, col)));
            ...
            for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
                ...
                if (pcmd->UserCallback) {
                    ...
                    if (clip_rect.x < static_cast<float>(fb_width) && clip_rect.y < static_cast<
                            float>(fb_height) &&
                        ...
                        // Bind texture, Draw
                        const GLuint textureHandle = convertImTextureIDToGLTextureHandle(
                            pcmd->TextureId);
